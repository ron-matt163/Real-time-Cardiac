...
def test_classifier(aggregated_features, aggregated_labels, num_epochs):
    if (aggregated_features is not None) and (aggregated_labels is not None):
        ...
        print("Testing the binary classifier")
        model_path = f'{model_directory}/smoteenn_acc91.pth'
        binclss_model = CNNLSTM1DWithAttentionTeacher().to(device)
        binclss_model.load_state_dict(torch.load(model_path, map_location=device))
        test_model(binclss_model, test_loader)
...
def test_model(model, test_loader, fold=-1, scored_indices=None):
...
def main(data_directory,
    ...
    test_classifier(aggregated_features, aggregated_labels, num_epochs)
