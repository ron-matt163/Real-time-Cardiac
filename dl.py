...
def load_dataset(data_directory):
    ...
    for idx in range(len(header_files)):
        ...
        features, cnt = process_ecg(recording, frequency, id)
...
def train_model(model, train_loader, weights, scored_indices, num_epochs):
    ...
    for epoch in range(num_epochs):  # num_epochs should be defined
        model.train()
        for i, (id, inputs, labels) in enumerate(train_loader):
            ...
            loss = custom_weighted_loss(outputs, labels, weights, scored_indices)
            loss.backward()
            optimizer.step()
...
def main(data_directory,
    ...
    for train_index, test_index in kf.split(aggregated_features):
        ...
        teacher_model = CNNLSTM1DWithAttentionTeacher(num_classes=129).to(device)
        ...
        train_model(teacher_model, train_loader, weights, scored_indices, num_epochs)

        model_path = f'{model_directory}/teacher_model.pth'
...
model_directory = '/Users/easonli/Desktop/physionet2021/main/model'
